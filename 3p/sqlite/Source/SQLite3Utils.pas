...
var
  SaveSeparator: Char;
begin
  SaveSeparator := {$IFNDEF FPC}FormatSettings.{$ENDIF}DecimalSeparator;
  {$IFNDEF FPC}FormatSettings.{$ENDIF}DecimalSeparator := '.';
  try
    Result := FloatToStr(Value);
  finally
    {$IFNDEF FPC}FormatSettings.{$ENDIF}DecimalSeparator := SaveSeparator;
  end;
end;
