...
begin
  if Assigned(fCommand) or Assigned(fRowSet) or (fColumnCount>0) or
    ...
    raise EOleDBException.CreateFmt('%s.Prepare should be called once',[ClassName]);
  inherited;
...
begin
  ...
  try
    ...
    if not Assigned(fCommand) then
      raise EOleDBException.CreateFmt('%s.Prepare should have been called',[ClassName]);
    if Assigned(fRowSet) or (fColumnCount>0) or
       (fColumnBindings<>nil) or (fParamBindings<>nil) then
      raise EOleDBException.CreateFmt('Missing call to %s.Reset',[ClassName]);
    // 2. bind parameters
