...

{
  ...
  ***** END LICENSE BLOCK *****
  ...
  - added dFirebird and dNexusDB kind of database in TSQLDBDefinition enumerate,
    including associated SQL requests to retrieve metadata
...
{$endif}
  ...
  // parameters and retrieve data from a statement result
  // - is used e.g. to
  ...
  public
    ...
    /// retrieve the advance indexed information of a specified Table
  ...
  public
    ...
    property ForceOnlyOneSharedConnection: boolean read fForceOneConnection write fForceOneConnection;
  end;
...
begin
  ...
  if fDBMS=dSQLite then begin // SQLite3 has a specific PRAGMA metdata query
    try
...
const
  ...
  TYPES: array[-1..high(PCHARS)] of TSQLDBFieldType = (
    ...
    ftCurrency,ftCurrency, ftUTF8, ftBlob, ftDate,
...
function TSQLDBConnectionProperties.FieldsFromList(const aFields: TSQLDBColumnDefineDynArray; aExcludeTypes: TSQLDBFieldTypes): RawUTF8;
var i,n: integer;
begin
  result := '';
...
begin
  if ForceOnlyOneSharedConnection then
    result := inherited GetMainConnection else begin
    EnterCriticalSection(fConnectionCS);
    try
...
begin
  case ColumnType(Col) of // will call GetCol() to check Col
    ...
    else     PIso8601(@result)^.From(Trim(ColumnUTF8(Col)));
  end;
end;
...
begin
  ...
  if PosEx('?',aSQL)>0 then begin
    ...
    repeat
      ...
      PAnsiChar(pointer(aNewSQL))[j] := ':';
      PAnsiChar(pointer(aNewSQL))[j+1] := AnsiChar(result and 15+65);
      PAnsiChar(pointer(aNewSQL))[j+2] := AnsiChar(result shr 4+65);
      inc(result);
      inc(i); // jump '?'
    until i=L;
  end else
