...
type
  ...
  published
    ...
    procedure Event(var Ctxt: TSQLRestServerCallBackParams);
  end;
...
procedure TFileServer.Event(var Ctxt: TSQLRestServerCallBackParams);
var E: integer;
begin
  if UrlDecodeInteger(Ctxt.Parameters,'EVENT=',E) and
     (E>ord(feUnknownState)) and (E<=ord(High(TFileEvent))) then begin
    AddAuditTrail(TFileEvent(E),'',RecordReference(Model,Ctxt.Table,Ctxt.ID));
    Ctxt.Success;
  end else
    Ctxt.Error;
end;
