...

{
  ...
  ***** END LICENSE BLOCK *****
    ...
    - VarDataToContext() will now bind '' text instead of null value
...
begin
  for i := 0 to high(Model.TableProps) do
    ...
    rRTree: // register all *_in() SQL functions
      sqlite3.create_function_v2(DB.DB,
    ...
    try
      with fStatementCache.Prepare(SQL)^ do begin
        ...
        for f := 0 to high(BlobFields) do begin
          VarDataFromValue(FieldValue(f),data);
          BlobFields[f].SetFieldVarData(Value,data); // OK for all blobs
        end;
        result := true;
      end;
    finally
...
begin
  ...
  if BlobFields<>nil then begin
    SQL := FormatUTF8('UPDATE % SET % WHERE ROWID=?;',
      [Table.RecordProps.SQLTableName,SQLTableUpdateBlobFields]);
    DB.Lock(SQL); // UPDATE for all blob fields -> no JSON cache flush, but UI refresh
    try
      with fStatementCache.Prepare(SQL)^ do begin
        for f := 1 to length(BlobFields) do begin
          BlobFields[f-1].GetFieldVarData(Value,data,temp); // OK for all blobs
          if data.VType=varAny then
            Bind(f,data.VPointer,data.VLongs[0]) else
...
begin
  case Res.VType of
    varNull:   sqlite3.result_null(Context);
    varInt64:  sqlite3.result_int64(Context,Res.VInt64);
    varDouble: sqlite3.result_double(Context,Res.VDouble);
    ...
    varString:
      if Res.VString=nil then
       sqlite3.result_text(Context,@NULCHAR,0,SQLITE_STATIC) else
       sqlite3.result_text(Context,Res.VString,-1,SQLITE_TRANSIENT_VIRTUALTABLE);
    varAny:    sqlite3.result_blob(Context,Res.VAny,Res.VLongs[0],SQLITE_TRANSIENT_VIRTUALTABLE);
    else begin
...
begin
  ...
  SQLITE_TEXT:  begin
    Res.VType := varString;
    Res.VString := sqlite3.value_text(Value);
  end;
  SQLITE_BLOB: begin
    Res.VType := varAny;
    Res.VLongs[0] := sqlite3.value_bytes(Value);
    Res.VAny := sqlite3.value_blob(Value);
  end;
  else
...
begin
  ...
  result := sqlite3.declare_vtab(DB,pointer(Table.Structure));
...
begin
  ...
  if TSQLVirtualTableCursor(pVtabCursor.pInstance).Column(-1,Res) then begin
    ...
    varCurrency,
    varDouble: pRowID := trunc(Res.VDouble);
...
procedure sqlite3InternalFreeModule(p: pointer); {$ifndef USEFASTCALL}cdecl;{$endif}
begin
