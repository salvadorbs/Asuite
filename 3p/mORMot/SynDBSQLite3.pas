...
type
  ...
  protected
    /// will force to use quoted table name if contains '.' or ' '
    function SQLTableName(const aTableName: RawUTF8): RawUTF8; override; 
...
function TSQLDBSQLite3ConnectionProperties.SQLTableName(
  const aTableName: RawUTF8): RawUTF8;
begin
  if (PosEx(' ',aTableName)>0) or (PosEx('.',aTableName)>0) then
...
begin
  if fBindShouldStoreValue then
    SynDBLog.Add.LogLines(sllSQL,pointer(SQLWithInlinedParams),self,'--');
