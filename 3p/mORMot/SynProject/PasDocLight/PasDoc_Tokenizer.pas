{
  ...
  @author(Arnaud Bouchez (arnaud@bouchez.info) - speed up)
...
type
  ...
  public
    ...
    property BeginPosition: integer read FBeginPosition;
  ...
  private
    function StreamPosition: integer;
  protected
    ...
    { if @link(IsCharBuffered) is true, this field contains the buffered
      character }
    BufferedChar: Char;
    { true if end of stream @link(FStream) has been reached, false otherwise }
    EOS: Boolean;
    { if this is true, @link(BufferedChar) contains a buffered character;
      ...
      character, not the next in the associated stream @link(FStream) }
    IsCharBuffered: Boolean;
    { current row in stream @link(FStream); useful when giving error messages }
    Row: Integer;
    ...
    function ReadToken(c: Char; const s: TCharSet; const TT: TTokenType;
      var t: TToken): Boolean;
...
const
  ...
  StandardDirectiveArray:
    array[Low(TStandardDirective)..High(TStandardDirective)] of string =
...
function KeyWordByName(const Name: string): TKeyword;
begin
  for result := KEY_AND to High(TKeyword) do
    if SameText(KeyWordArray[result],Name) then
      exit;
...
function StandardDirectiveByName(const Name: string): TStandardDirective;
begin
  for result := SD_ABSOLUTE to High(TStandardDirective) do
    if SameText(StandardDirectiveArray[result],Name) then
      exit;
...
begin
  HasData := IsCharBuffered or (StreamPos < StreamSize);
end;

{ ---------------------------------------------------------------------------- }

function TTokenizer.StreamPosition: integer;
begin
  if IsCharBuffered then
    Result := StreamPos - 1 else
    Result := StreamPos;
end;
      ...
      case c of
        ...
        '/': begin
            ...
            case c of
              '/': begin
                  ConsumeChar;
                  Result := ReadCommentType3; // TOK_COMMENT_CSTYLE
                end;
            else
...
begin
  ...
  repeat
    if not (StreamPos < StreamSize) then begin
      ReleaseToken;
