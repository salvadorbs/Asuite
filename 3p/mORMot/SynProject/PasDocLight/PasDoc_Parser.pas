{ @abstract(provides all the parsing functionality of pasdoc)
  ...
  @cvs($Date: 2007-01-25 13:57:11 +0100 (czw, 25 sty 2007) $)
...
type
  ...
  private
    ...
    function ParseConstant(const U: TPasUnit): TPasConstant;
    ...
    procedure ParseUses(const U: TPasUnit; ImplementationSection: boolean);
    ...
    procedure ParseUnit(U: TPasUnit; UnitUsesAlsoInImplementation: boolean = false);
  ...
  public
    ...
    procedure ParseUnitOrProgram(var U: TPasUnit; UnitUsesAlsoInImplementation: boolean = false);
...
function TParser.ParseConstant(const U: TPasUnit): TPasConstant;
begin
  result := TPasConstant.Create;
  result.Name := GetAndCheckNextToken(TOK_IDENTIFIER);
  DoMessage(5, mtInformation, 'Parsing constant %s', [result.Name]);
  result.RawDescriptionInfo^ := GetLastComment;
  result.FullDeclaration := result.Name;
  SkipDeclaration(result, false);
  U.AddConstant(result);
  ItemsForNextBackComment.ClearAndAdd(result);
end;
          ...
          begin
            case Mode of
              ...
              MODE_RESSTRING:
                begin
                  Scanner.UnGetToken(T);
                  ParseConstant(U).IsResourceString := true;
                end;
              MODE_TYPE:
                begin
                  Scanner.UnGetToken(T);
...
procedure TParser.ParseUnit(U: TPasUnit; UnitUsesAlsoInImplementation: boolean = false);
var
  T: TToken;
begin
  ...
  { parse the uses clauses in the implementation section of that unit }
  if not UnitUsesAlsoInImplementation then
    exit;
  T := GetNextToken;
  try
...
procedure TParser.ParseUnitOrProgram(var U: TPasUnit; UnitUsesAlsoInImplementation: boolean = false);
var
  t: TToken;
begin
  U := TPasUnit.Create;
  try
    ...
    if U.IsUnit then
      ParseUnit(U,UnitUsesAlsoInImplementation) else
      begin
...
procedure TParser.ParseUses(const U: TPasUnit; ImplementationSection: boolean);
var UsesName: string;
    T: TToken;
begin
  ...
  repeat
    UsesName := GetAndCheckNextToken(TOK_IDENTIFIER, true);
    U.UsesUnits.Append(UsesName);
    if not ImplementationSection then
      U.InterfaceUsesUnits.Append(UsesName);
...
function TParser.PeekNextToken(out WhitespaceCollector: string): TToken;
  ...
  begin
    ...
    if (CommentInfo.Content<>'') and (CommentMarkers<>'') and
       // test /// only if not // with /// before
       ((not IsLastComment and not LastCommentWasCStyle) or (LastCommentInfo.Content='') or
        (t.MyType<>TOK_COMMENT_CSTYLE) or (not LastCommentWasCStyle)) then begin
      i := Pos(CommentInfo.Content[1],CommentMarkers);
      if i>0 then begin // first char is in '/{*'
        Delete(CommentInfo.Content, 1, 1);
        WasMarker := true;
      end else
        WasMarker := false;
    ...
    if (CommentInfo.Content<>'') and (Pos(CommentInfo.Content[1],CommentMarkers)>0) then begin
      Delete(CommentInfo.Content, 1, 1);
    end;
