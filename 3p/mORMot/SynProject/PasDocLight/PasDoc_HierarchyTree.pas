{
  @cvs($Date: 2005-08-19 01:25:43 +0200 (pi±, 19 sie 2005) $)
...
type
  ...
  public
    ...
    function HasParent(const aParentName: string): boolean;
...
function TPasItemNode.FindItem(const AName: string): TPasItemNode;
var i: Integer;
begin
  Result := nil;
  for i := 0 to FChildren.Count - 1 do begin
    if SameText(TPasItemNode(FChildren[i]).Name,AName) then begin
...
function TPasItemNode.HasParent(const aParentName: string): boolean;
var
  Node: TPasItemNode;
begin
  result := true;
  Node := self;
  while Node<>nil do
    if SameText(Node.Name,aParentName) then
      exit else
      Node := Node.Parent;
  result := false;
end;
