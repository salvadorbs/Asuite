...
{$ifdef DWPL} // special DWPL compatible mode
  {$undef FullDebugMode}
  ...
  {$undef UseCustomFixedSizeMoveRoutines} // spare some code size on ZFLinux
  {$undef AssumeMultiThreaded} // IsMultiThread := false under DWPL -> faster
  {$undef EnableMMX}
  {$undef Align16Bytes}
  {$undef UseRuntimePackages}
{$endif}
...
{$else} MoveX8L4(APointer^, Result^, ANewSize);
{$endif}
{$else} System.Move(APointer^, Result^, ANewSize);
{$endif}
        {Free the old block}
        FastFreeMem(APointer);
      end;
...
{$else}   System.Move(APointer^, Result^, LOldAvailableSize);
{$endif}
          {Free the old block}
          FastFreeMem(APointer);
        end;
