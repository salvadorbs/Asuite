...
type
  ...
  public
    ...
    // - BlobPrefetchSize and StatementCacheSize field values of the associated
...
begin
  ...
  if (PInteger(@self)^=0) and (PInteger(PtrInt(@self)+3)^=0) then
    ...
    if Y>3000 then // avoid integer overflow -> stored as ""
      result := 2 else begin
...
var msg: array[0..3071] of AnsiChar;
    L, ErrNum: integer;
    Error: RawUTF8;
begin
  msg[0] := #0;
...
function SimilarCharSet(aCharset1, aCharset2: Integer): Boolean;
var i1,i2: integer;
begin
  result := true;
            ...
            SQLCS_IMPLICIT: begin
              ...
              if ColumnValueDBCharSet=SQLCS_IMPLICIT then
                ...
                {$ifdef DELPHI5OROLDER}
                  Log.Log(sllWarning,'Column has wrong charset -> possible data loss');
                {$else}
                  Log.Log(sllWarning,'Column "%" has % (%) charset - expected % (%) '+
